...
from contextforge_cli.utils.ai_docs_utils.api import (
...
from contextforge_cli.utils.file_functions import tilda
...
async def _extend_docs(
    repo_name: str, messages: list[dict[str, str]], message: str
) -> None:
...
async def agenerate_module_docs(module_path: str) -> None:
    ...
    logger.debug(
        "Received response from Claude Opus",
    ...
    logger.info(
        "Writing documentation", docs_path=docs_path, content_length=len(docs_text)
    )
    ...
    try:
        ...
        logger.info(
            "Successfully generated module documentation",
